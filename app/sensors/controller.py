...
@router.get("/near")
def get_sensors_near(latitude: float, longitude: float, radius: int, redis: Session = Depends(get_redis_client), mongodb_client: MongoDBClient = Depends(get_mongodb_client)):
    return repository.get_sensors_near(mongodb=mongodb_client, latitude=latitude, longitude=longitude, radius=radius, redis=redis)
...
def record_data(sensor_id: int, data: schemas.SensorData, db: Session = Depends(get_db), redis_client: RedisClient = Depends(get_redis_client)):
    return repository.record_data(redis=redis_client, db=db, sensor_id=sensor_id, data=data)
